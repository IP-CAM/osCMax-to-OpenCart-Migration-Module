$(document).ready(function(){
	...
	function clearFtpInfo(install){
		...
		$.ajax({
			cache: false,
			url: SELF_PATH +'clearFtpInfo'+'&token='+opencartToken
		});
	}
	...
	function saveLoginStatus(status,email,encPass){
		$.ajax({
			cache: false,
			url: SELF_PATH+"saveLoginStatus&status="+status+"&email="+encodeURIComponent(email)+'&encPass='+encPass+'&token='+opencartToken,
			success:function(){
	...
	function saveToken(token){
		...
		$.ajax({
			url: SELF_PATH + "saveToken&c2c_token="+token+'&token='+opencartToken,
			success: function(){
	...
	function sendBridgeRequest(install){
		...
		$.ajax({
			cache: false,
			url: SELF_PATH + install+'Bridge'+'&token='+opencartToken,
			success: function(){
				if (install == 'install'){
	...
	function saveFtpInfo(host,user,pass,dir){
		...
		$.ajax({
			cache: false,
			url: SELF_PATH + "saveFtp&host="+host+"&user="+user+"&pass="+encodeURIComponent(pass)+"&dir="+dir+"&token="+opencartToken,
